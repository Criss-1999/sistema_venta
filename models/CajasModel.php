...
class CajasModel extends Query
{
  ...
  public function registrarGasto($monto, $descripcion, $destino, $id_usuario)
  {
  ...
  {
    $sql = "SELECT SUM(d.monto) as total FROM detalle_apartado d INNER JOIN apartados a ON d.id_apartado = a.id 
    WHERE d.apertura AND a.id_usuario = $id_usuario";
    return $this->select($sql);
  }

  public function getAbonos($id_usuario)
  {
    $sql = "SELECT SUM(a.abono) as total FROM abonos a INNER JOIN creditos c ON a.id_credito = c.id INNER JOIN ventas v
    ON c.id_venta = v.id WHERE a.apertura = 1 AND v.id_usuario = $id_usuario";
    return $this->select($sql);
  }
  ...
  public function getTotalVenta($id_usuario)
  {
