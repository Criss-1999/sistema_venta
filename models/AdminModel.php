...
class AdminModel extends Query
{
    ...
    public function actualizar($ruc, $nombre, $telefono, $correo, $direccion, $impuesto, $mensaje, $id)
    {
        $sql = "UPDATE configuracion SET ruc = ?,nombre = ?,telefono = ?,correo = ?,direccion = ?,impuesto = ?,mensaje = ? WHERE id = ?";
        $array = array($ruc, $nombre, $telefono, $correo, $direccion, $impuesto, $mensaje, $id);
    ...
    public function calcularVentasCompras($table, $desde,$hasta, $id_usuario)
    {
        ...
        SUM(IF(MONTH(fecha) = 12, total, 0)) AS dic FROM $table
        WHERE fecha BETWEEN '$desde' AND '$hasta' AND id_usuario = $id_usuario";
        return $this->select($sql);
    }
    ...
    public function topProductos($cantidad)
    {
        ...
        ORDER BY p.ventas DESC LIMIT $cantidad";
    ...
    public function calcularGastos( $desde,$hasta, $id_usuario)
    {
        ...
        SUM(IF(MONTH(fecha) = 12, monto, 0)) AS dic FROM gastos
        WHERE fecha BETWEEN '$desde' AND '$hasta' AND id_usuario = $id_usuario";
        return $this->select($sql);
    }
