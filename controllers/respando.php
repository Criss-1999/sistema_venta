...
class Ventas extends Controller
{
        ...
        if (!empty($datos['productos'])) {
            ...
            if (empty($idCliente)) {
                ...
                if (empty($verifcarCaja['monto_inicial'])) {
                    ...
                    if ($datos['pago'] <  $total) {
                        $res = array('msg' => 'El Pagar con tiene que ser mayor o igual al total', 'type' => 'warning');
                    } else {  // Cambio el "else if" por un simple "else"
    ...
    //enviar ticked al correo del cliente
    public function enviarCorreo($idVenta)
    {
        ...
        ob_start();
        $data['title'] = 'Reporte';
        $this->views->getView('ventas','ticked_cliente', $data);
        $html = ob_get_clean();
        if (!empty($data)) {
