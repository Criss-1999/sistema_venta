...
class Cotizaciones extends Controller
{
    ...
    public function registrarCotizacion()
    {
        ...
        if (!empty($datos['productos'])) {
            ...
            if (empty($idCliente)) {
                $res = array('msg' => 'INGRESE EL CLIENTE', 'type' => 'warning');
            } else if (empty($metodo)) {
                $res = array('msg' => 'INGRESE EL METODO', 'type' => 'warning');
            } else if (empty($validez)) {
                $res = array('msg' => 'INGRESE LA VALIDEZ', 'type' => 'warning');
            } else {
                ...
                if ($cotizacion > 0) {
                    $res = array('msg' => 'COTIZACION GENERADA', 'type' => 'success', 'idCotizacion' => $cotizacion);
                } else {
                    $res = array('msg' => 'ERROR AL GENERAR LA COTIZACION', 'type' => 'error');
                }
            }
        } else {
