...
class Categorias extends Controller
{
  ...
  public function registrar()
  {
    if (isset($_POST['nombre'])) {
      ...
      if (empty($categoria)) {
        ...
        if ($id == '') {
          $vereficar = $this->model->getValidar('categoria', $categoria, 'registrar', 0);
          if (empty($vereficar)) {
            $data = $this->model->registrar($categoria);
            if ($data > 0) {
              $res = array('msg' => 'CATEGORIA REGISTRADA', 'type' => 'success');
            } else {
          ...
          $vereficar = $this->model->getValidar('categoria', $categoria, 'actualizar', $id);
          if (empty($vereficar)) {
            $data = $this->model->actualizar($categoria, $id);
            if ($data > 0) {
              $res = array('msg' => 'CATEGORIA ACTUALIZADA', 'type' => 'success');
            } else {
  ...
  public function restaurar($idCategoria)
  {
    if (isset($_GET) && is_numeric($idCategoria)) {
      ...
      if ($data == 1) {
        $res = array('msg' => 'CATEGORIA HA SIDO RESTAURADA', 'type' => 'success');
      } else {
