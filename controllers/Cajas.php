...
class Cajas extends Controller
{
    ...
    public function abrirCaja()
    {
        ...
        if (empty($datos['monto'])) {
            $res = array('msg' => 'INGRESE EL MONTO INICIAL', 'type' => 'warning');
        } else {
    ...
    public function registrarGasto()
    {
        if (isset($_POST['monto']) && isset($_POST['descripcion'])) {
            if (empty($_POST['monto'])) {
                ...
                $res = array('msg' => 'INGRESE LA DESCRIPCION', 'type' => 'warning');
            } else {
                $monto = strClean($_POST['monto']);
                $verificarMonto = $this->getDatos();
                $Ingresos = $verificarMonto['ingresos'] + $verificarMonto['montoInicial'];
                if ($monto > $Ingresos) {
                    ...
                    $data = $this->model->registrarGasto($monto, $descripcion, $destino, $this->id_usuario);
                    if ($data > 0) {
                        if (!empty($name)) {
                        ...
                        $res = array('msg' => 'ERROR AL REGISTRAR GASTO', 'type' => 'error');
                    }
                }
            }
        }
    ...
    public function listarGastos()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
          ...
          <img class="img-thumbnail" src="' . BASE_URL . $data[$i]['foto'] . '" width="200"></a>';
        }
        echo json_encode($data);
        die();
    }
    ...
    {
        $data = $this->getDatos();
        $ventas = $this->model->getTotalVenta($this->id_usuario);
    ...
    public function historialReporte($idCaja)
    {
        ob_start();
