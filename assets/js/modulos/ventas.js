      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (data.length > 0) {
            ...
            errorCliente.textContent = "EL NOMBRE NO EXISTE";
          }
        },
      });
    },
            ...
            setTimeout(() => {
              Swal.fire({
                ...
                if (metodo.value == "CONTADO") {
                  ...
                  if (result.isConfirmed) {
                    const ruta =
                      base_url + "ventas/reporte/ticked-credito/" + res.idVenta;
                    window.open(ruta, "_blank");
                  } else if (result.isDenied) {
                    const ruta =
                      base_url + "ventas/reporte/factura-credito/" + res.idVenta;
                    window.open(ruta, "_blank");
                  }
                }
...
function mostrarProducto() {
 ...
 /* Cambiar la ruta correcto para credito y contado */
function verReporte(idVenta) {
  Swal.fire({
    title: "¿Desea Vizualizar el Reporte?",
    showDenyButton: true,
    showCancelButton: true,
...
function enviarComprobante(idVenta) {
  Swal.fire({
    title: "¿Enviar factura de venta al Correo?",
    text: "¡Asegurese que exista el correo!",
    icon: "warning",
    showCancelButton: true,
    confirmButtonColor: "#3085d6",
    cancelButtonColor: "#d33",
